...
def test_prestage_context_in_essential_and_full_profiles():
    mod = _load_server()
    assert "prestage_context" in mod._ESSENTIAL_TOOLS
    assert "prestage_context" in mod._PROFILE_DEFINITIONS["full"]
    assert "prestage_context" in mod._PROFILE_DEFINITIONS["power"]
